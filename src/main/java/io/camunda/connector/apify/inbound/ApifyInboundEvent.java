...
/**
 * Represents the webhook payload received from Apify when an Actor or Task run completes.
 * 
 * This DTO maps the JSON payload sent by Apify webhooks to Java fields.
 * The payload contains information about the event type and the resource (run) that triggered it.
 */
...
public record ApifyInboundEvent(
     ...
     * Examples: "ACTOR.RUN.SUCCEEDED", "ACTOR.RUN.FAILED", "ACTOR.RUN.TIMED-OUT", "ACTOR.RUN.ABORTED"
     */
    ...
    // TODO: check if it's all necessary data, compare it with power automate for instance
    /**
